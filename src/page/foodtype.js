...
function FoodType() {
  ...
  const foodToday = searchParams.get("today");
  ...
  const fetchItems = useCallback(
    ...
    [apiUrl, foodType, foodToday]
  );

  // foodType 또는 foodToday가 바뀌면 초기화 후 첫 페이지 불러오기
  useEffect(() => {
    setPageNumber(1);
    setHasMore(true);
    fetchItems(1);
  }, [foodType, foodToday, fetchItems]);
