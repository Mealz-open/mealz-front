...
function MyFinishedReservation() {
  const apiUrl = process.env.REACT_APP_API_URL;

  const [benefioiaryId, setBenefioiaryId] = useState(null);
  ...
  useEffect(() => {
    async function fetchUserInfo() {
      try {
        ...
        setBenefioiaryId(data.memberId);
      } catch (e) {
  ...
  const fetchTrades = useCallback(
    async (page) => {
      if (!benefioiaryId) return;
      setLoading(true);
      setError(null);

      try {
        const params = new URLSearchParams({
          benefioiaryId: benefioiaryId,
          pageNumber: page,
          pageSize: 20,
  ...
  useEffect(() => {
    ...
    setHasMore(true);
    fetchTrades(1);
  }, [benefioiaryId, fetchTrades]);

  // 페이지 이동 시 추가 데이터
  useEffect(() => {
    if (pageNumber === 1 || !benefioiaryId) return;
    fetchTrades(pageNumber);
  }, [pageNumber, fetchTrades, benefioiaryId]);
...
export default MyFinishedReservation;